...
function findSafeSpawn(game){
  // Use noise directly to find a land position — no chunk generation needed
  // This avoids the issue where chunk generation overwrites tiles
  const SAFE_BIOMES = ['plains','forest','dense_forest','jungle','swamp','tundra','desert'];

  // Spiral outward checking elevation noise directly
  for(let r=0; r<=50; r++){
    for(let dy=-r; dy<=r; dy++){
      for(let dx=-r; dx<=r; dx++){
        if(Math.abs(dx)!==r && Math.abs(dy)!==r) continue;
        const wx=dx, wy=dy;
        // Check elevation — if above water threshold, it's land
        const elev=octaveNoise(wx,wy,game.seed,5,0.5,0.012);
        if(elev>=0.33 && elev<0.76){ // above water but below stone threshold
          // Generate the chunk for this position
          const cx=Math.floor(wx/CHUNK_SIZE), cy=Math.floor(wy/CHUNK_SIZE);
          game.getChunk(cx,cy);
          const tile=game.getTile(wx,wy);
          // Double-check: must not be water, solid, or tree
          if(!WATER_TILES.has(tile) && !SOLID_TILES.has(tile) && tile!==T.TREE){
  ...
  // Absolute fallback — generate chunks in a wide area and find any land
  for(let cr=0;cr<=10;cr++){
    for(let cy=-cr;cy<=cr;cy++) for(let cx=-cr;cx<=cr;cx++) game.getChunk(cx,cy);
    for(let y=-cr*CHUNK_SIZE;y<=cr*CHUNK_SIZE;y++){
      for(let x=-cr*CHUNK_SIZE;x<=cr*CHUNK_SIZE;x++){
...
async function startNewGame(){
  ...
  // Step 2 — generate initial spawn chunks (findSafeSpawn will generate more if needed)
  showLoading('Generating world...', 25);
  await yieldFrame();
  try{
    for(let dy=-1;dy<=1;dy++){
      dbg('Generating chunk row dy='+dy);
      for(let dx=-1;dx<=1;dx++) game.getChunk(dx,dy);
      await yieldFrame();
    }
    dbg('Initial chunks generated OK');
  } catch(e){
    dbg('FAILED generating chunks: '+e.message, true);
    hideLoading(); alert('World gen failed: '+e.message); return;
  }

  // Step 3 — find safe spawn (generates more chunks as needed)
  showLoading('Finding spawn point...', 55);
  await yieldFrame();
  try{
    const [spx, spy] = findSafeSpawn(game);
    game.spawnX = spx; game.spawnY = spy;
    dbg('Spawn found at '+spx.toFixed(1)+','+spy.toFixed(1)+' tile='+game.getTile(spx|0,spy|0));
