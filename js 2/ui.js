...
class UI {
  ...
  _drawInventory(ctx,p,W,H,anim=1){
    ...
    const pw=COLS*SLOT+PAD*2+160;
    const ph=Math.min(rows*SLOT+80, H-60);
    const px=W/2-pw/2, py=H/2-ph/2;

    // Fade-only animation — NO scale (scale causes mirror effect)
    ctx.save();
  ...
  endDrag(ex, ey, p, sendAction){
    ...
    for(const r of (this.armourRects||[])){
      if(ex>=r.x&&ex<=r.x+r.w&&ey>=r.y&&ey<=r.y+r.h){
        if(ARMOUR_DEFENCE[id]||id==='warm_coat'||id==='fur_coat'){
          sendAction({type:'equip',slot:r.slot,itemId:id});
          // Clear hotbar slot if dragged from hotbar
          if(fromHotbar!==undefined) this.hotbar[fromHotbar]=null;
          return;
        }
      }
    }
    ...
    for(const r of this.invRects){
      if(ex>=r.x&&ex<=r.x+r.w&&ey>=r.y&&ey<=r.y+r.h){
        // Dragged from hotbar into inventory area — clear the hotbar slot
        if(fromHotbar!==undefined){
          this.hotbar[fromHotbar]=null;
        }
        // Swap with the inventory item if different
        if(r.id!==id&&fromHotbar!==undefined){
          this.hotbar[fromHotbar]=r.id; // put the inv item in hotbar
        }
        return;
      }
    }

    // Dropped on inventory panel background (empty space) — clear hotbar slot
    if(this.showInv && fromHotbar!==undefined){
      // Check if drop is inside the inventory panel area
      const SLOT=46, COLS=8, PAD=12;
      const inv=p?.inventory||{};
      const rows=Math.max(6,Math.ceil(Object.keys(inv).length/COLS));
      const pw=COLS*SLOT+PAD*2+160;
      const ph=Math.min(rows*SLOT+80, window.innerHeight-60);
      const px=window.innerWidth/2-pw/2, py=window.innerHeight/2-ph/2;
    ...
    // Dropped completely outside everything — drop item on ground
    if(fromHotbar!==undefined){
      // Dragged from hotbar to outside — clear slot, drop on ground
      this.hotbar[fromHotbar]=null;
    }
